...
// ---------------------------------------------------------------------
// Signature Block (STRUCTURAL ONLY)
// ---------------------------------------------------------------------
...
useEffect(() => {
  ...
  // ---------------------------------------------------------------------
// Auto insert signature block when signatory exists
// ---------------------------------------------------------------------
  useEffect(() => {
    if (!editor) return;

    // No signatory data → do nothing
    if (!signatory?.fullName && !signatory?.signatureUrl) return;

    const json = editor.getJSON()

    const hasSignatureBlock = (json.content ?? []).some(
      (n) => n.type === "signaturesBlock"
    )
  ...
  if (isCanvas) {
    return (
      <div className="flex-1 overflow-visible px-4 py-8">
        <div id="formyxa-doc-top" />
        <div className="max-w-[780px] mx-auto">
          {layout.shellVariant === "page" ? (
            editor ? (
              <div className={`offer-root ${effectiveDesignKey}`}>
                <DocumentPageShell
                  editor={editor}
                  layout={layout}
                  brand={brand}
              ...
              <div className="bg-white rounded-2xl shadow-lg px-14 py-12 min-h-[11in] flex items-center justify-center">
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {effectiveMode === "document" ? (
        <main className="flex-1 flex justify-center items-start py-6 px-3 bg-slate-50">
          <div className="bg-slate-200/80 rounded-xl p-4">
            {layout.shellVariant === "page" ? (
              ...
              <div
                className="bg-white shadow-[0_0_0_1px_rgba(15,23,42,0.06),0_18px_40px_rgba(15,23,42,0.08)] rounded-md mx-auto w-[794px] min-h-[1123px] px-16 py-12 flex flex-col"
